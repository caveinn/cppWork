...
string SchedulingSystem::finalResultsTable() const
{
  ...
  for (Pid pid = 0; pid < numProcesses; pid++)
  {
    ...
    out << setw(4) << left << p.name << " "
        << setw(4) << left << p.arrivalTime << " "
        << setw(4) << left << p.serviceTime << " "
        << setw(4) << left << p.startTime << " "
        << setw(4) << left << p.endTime << " "
        << setw(4) << left << turnaroundTime << " "
        << setw(8) << left << setprecision(4) << fixed << trtsRatio << endl;
  }
...
void SchedulingSystem::loadProcessTable(string simfilename)
{
  ...
  if ( (pid != numProcesses) or (not processfile.eof()) )
  {
    ...
    msg << "<SchedulingSystem::loadPageStream> Error, did not see expected"
        << " number of processes in process table, expected: "
        << numProcesses
        << " received: "
        << pid << endl;
    throw SimulatorException(msg.str());
 ...
 */
void SchedulingSystem::generateRandomProcessTable(int numProcesses, double arrivalProbability,
                                                  int maxServiceTime, int seed)
{
